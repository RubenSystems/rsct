...
pub fn recieve_once(socket: &UdpSocket) -> Result<PacketContainer, RecieveError> {
    let mut recieve_buffer = [0u8; PACKET_SIZE];

    let (len, addr) = socket.recv_from(&mut recieve_buffer).map_err(|e| RecieveError::RecieveFromError(e.to_string()))?;
